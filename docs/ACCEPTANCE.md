...
| 17 | Mobile cold start < 2.5 s on a mid-tier Android | ⚠️ | **The app exists; the measurement does not.** `apps/mobile` is Expo Router against ADR-0002, covering §18.2: splash, onboarding, auth, role picker, home, search, filters, listing detail, my stable, the horse record with §20.4's timeline, both wizards, health records, competitions, §6's transfer, my listings with §5's transitions, inbox, thread, enquiries, saved items, saved searches, notifications, services, jobs and applications, professionals, public profiles, verification, plans, settings and reporting. What cannot be done here is *time a cold start*: there is no simulator and no device. Earlier revisions of this row said the app "type-checks" while `apps/mobile` did not exist; that was false and was corrected before this. |
...
| # | Criterion | State | Notes |
|---|---|---|---|
| 21 | ≥ 70 % on API business logic; 100 % on lifecycle, quality score, trust score, limits, webhook handlers | ⚠️ | **Split, and measured two ways.** The named rules live in `packages/shared-types` and are at **100 % statements** each (listing-lifecycle, quality-score, trust, limits, billing; package total 96.9 %, 108 tests). The NestJS services are at **4.5 % statements**. Measured a second way — every request recorded through a proxy while every suite executed — route coverage went from **79 of 137 (57 %)** to **137 of 137 (100 %)** once `scripts/coverage-sweep.mjs` was written to call the 58 nobody called. That sweep found four more endpoints that had never worked: adding a credential, transferring a horse, archiving a horse, and (earlier) pausing a listing. The 70 % statement line is still **not met** — endpoint coverage is not unit coverage — but no route is now completely unexercised. |
| 22 | E2E covers signup → horse → listing → search → inquiry → reply → close | ✅ | On mobile, `scripts/mobile-e2e.mjs` walks **27 steps** through the Expo web export in Chromium against the live API: browse, register, pick a role, hit §3.3's publishing gate, add a horse, upload a photo through §10.1's three-step pipeline, read its health log, add a competition result and see it in §20.4's timeline, reach §6's transfer, save a listing, message a seller and land in the thread, save a search, open notifications, apply to a job, enquire about a service, rename the account, sign out and sign back in with the horse still there. All 27 pass, the last two covering the equipment marketplace: listing a product, publishing it, and filtering the catalogue. On web, `scripts/e2e.mjs` drives the whole journey in Chromium: register, horse, 8 photos and a video through the real media pipeline, publish, find it by searching **in the browser**, click through to the detail page, assert the title, price and §14.5 safety card, re-render the page **with JavaScript disabled** to prove §19.2's indexability, then inquiry → reply → close and confirm it left the index. `scripts/web-account.mjs` covers the signed-in half in **16 steps** — register in the browser, record a horse, log a health entry and see the reminder it schedules, add a competition result and find both on §20.4's timeline, compose a listing as a draft, hit §3.3's identity gate on publishing a product and get routed to the ladder rather than the paywall, publish once verified, find that product back through §11's search, open every account screen, and answer a message — and asserts that the session token is unreadable from script and that a signed-out visitor cannot open an account page. `scripts/web-shot.mjs` then renders all **34 routes** against the live API and fails on any page error. Both run in CI against a real Postgres and a real API. |
